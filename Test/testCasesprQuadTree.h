...
//testcase 5:
vector<QPoint*> pointsTestCase5(){
    vector<QPoint*> vQPoint;
    vQPoint.push_back(new QPoint(1,2,5));
    vQPoint.push_back(new QPoint(3,4,6));
    return vQPoint;

}

void prQuadTreeTestCase5()
{
    cout<<"\n\nprQuadTree Test Case 5"<<endl;
    prQuadTree *prQT = new prQuadTree(5,5,10,10);
    vector<QPoint*> testQPoints=pointsTestCase5();
    for(int i=0;i<testQPoints.size();i++) {
        prQT->insert(testQPoints[i]);
    }
    prQT->print();
    cout<<"\n\n\n\n\n"<<endl;
    cout<<"\n\nprQuadTree 5 - Query Range 1"<<endl;
    printprQuadTreeRange(prQT,0,0,10,10);
